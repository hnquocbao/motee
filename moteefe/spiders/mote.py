...
class MoteSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        for product_main in campaign_products:
            ...
            color_ids = product_main.get('color_ids')
            for index, color_id in enumerate(color_ids, start=0):
                loader = ItemLoader(item=MoteefeItem())
                loader.add_value('Variant_Price', product_main.get('price'))
                loader.add_value('Variant_Compare_At_Price', product_main.get('pre_discounted_price'))
                for product_references in product_list:
                    if product_references.get('product_id') == product_main_id:
                        # Get Handle
                        loader.add_value('Handle', product_references.get('slug'))
                        if index == 0:
                            # Get Title
                            name = product_references.get('name')
                            name.replace("common:products.names.", "")
                            loader.add_value('Title', product_names.get(name))
                            # Get description
                            details = product_references.get('details')
                            details.replace("common:products.descriptions.", "")
                            loader.add_value('Body_HTML', product_descriptions.get(details))
                            # Get Type
                        ...
                        loader.add_value('Option_1_Value', product_names.get(name))
                        # Get color
                        for color in product_references.get('colors'):
                            if color.get('id') == color_id:
                                ...
                                loader.add_value('Option_2_Value', color.get('name'))
                                 # Get Variant SKU
                                loader.add_value('Variant_SKU', product_references.get('slug')+"-"+color.get('name'))
                                break
                        # Get Sizes
                        sizes = []
                        ...
                        loader.add_value('Option_3_Value', sizes)
                        break
